...
class TestBequePerformance:
    ...
    async def test_large_scale_fifo_ordering_10k(self):
        ...
        async with Beque(
            ...
            print(f"Added {num_items} items in {add_time:.2f}s ({num_items/add_time:.0f} items/s)")
        ...
        for i, item in enumerate(processed_items):
            ...
            assert item == expected, f"Order violation at index {i}: expected {expected}, got {item}"
    ...
    async def test_large_scale_fifo_ordering_50k(self):
        ...
        for i in sorted(sample_indices):
            ...
            assert actual == expected, f"Order violation at index {i}: expected {expected}, got {actual}"
    ...
    async def test_concurrent_large_scale_fifo(self):
        ...
        async with Beque(
            ...
            await asyncio.gather(*[
                producer(pid, items_per_producer) 
        ...
        async with Beque(
            ...
            for i in range(num_items):
                ...
                if i % 5000 == 0:
                    ...
                    print(f"Items {i}: Queue size {beque.stats['queued']}, Memory: +{memory_increase:.1f}MB")
    ...
    async def test_high_throughput_performance(self):
        ...
        async with Beque(
            ...
            for i in range(0, num_items, 100):
                chunk = [f"throughput_item_{j}" for j in range(i, min(i + 100, num_items))]
                await beque.add_many(chunk)
        ...
        print(f"Total time: {process_time:.3f}s ({total_items/process_time:.0f} items/s)")
        print(f"Batches processed: {len(processed_batches)}")
        
        assert total_items == num_items
        assert len(processed_batches) < num_items / 10  # Should use batching efficiently
    ...
    async def test_burst_handling_performance(self):
        ...
        async with Beque(
            ...
            for burst in range(5):
                ...
                print(f"Burst {burst + 1}: Added {burst_size} items in {burst_add_time:.2f}s")
        ...
        for burst in range(5):
            burst_items = [item for item in processed_items if item.startswith(f"burst_{burst}_")]
    ...
    async def test_variable_item_size_performance(self):
        ...
        avg_batch_size = sum(processed_sizes) / total_batches if total_batches > 0 else 0
        
        print(f"Variable size test: {total_batches} batches, avg batch size: {avg_batch_size:.0f} chars")
...
class TestBequeLargeBatchBehavior:
    ...
    async def test_single_large_batch_processing(self):
        ...
        for i in range(0, min(1000, large_batch_size)):  # Sample check
    ...
    async def test_multiple_large_batches(self):
        ...
        async with Beque(
            ...
            for batch_num in range(num_batches):
                batch_items = [
                    f"batch_{batch_num:02d}_item_{i:05d}" 
        ...
        print(f"Multiple large batches: {num_batches} batches of {batch_size} items each")
        ...
        async with Beque(
            ...
            async def large_batch_producer():
                ...
                for batch_num in range(3):
                    large_batch = [f"large_batch_{batch_num}_item_{i:04d}" for i in range(800)]
...
class TestBequeStressScenarios:
    ...
    async def test_extreme_batch_size_ratios(self):
        ...
        for batch_size, item_count in test_cases:
            ...
            print(f"  Completed in {duration:.2f}s, processed {processed_count}/{item_count} items")
    ...
    async def test_queue_buildup_and_drainage(self):
        ...
        async with Beque(
            ...
            print(f"Queue buildup: {buildup_stats['queued']} items, {buildup_stats['failures']} failures")
            ...
            print(f"Final stats: {final_stats['items']} items processed, {final_stats['queued']} queued")
        ...
        for i, item in enumerate(processed_items):
            ...
            assert item == expected, f"Order violation at {i}: expected {expected}, got {item}"
        ...
        async with Beque(
            ...
            while time.monotonic() - start_time < 3.0:  # 3 second test
                ...
                if current_time % 1.0 < 0.5:  # High load for first half of each second
                    ...
                    items = [f"stability_item_{total_added + i}" for i in range(burst_size)]
                    await beque.add_many(items)
                ...
                if total_added % 1000 == 0:
                    stats = beque.stats
                    print(f"Added {total_added}, processed {processed_count}, queued {stats['queued']}")
